function [pval, table, validCode] = circ_wwtest_ModByBAG(varargin)
...
validCode = checkAssumption(rw,mean(pn));
...
%   end
  ...
  function prepareOutput
    ...
    if na > 2 %Changed from 1 to 2 since I added the validCode output variable;
...
function validCode = checkAssumption(rw,n)
validCode = 0; %Default is that assumptions are valid for the test
  if n >= 11 && rw<.45
      validCode = 1;
